...
class DeseaseSummary(models.TransientModel):
    ...
    _name = 'hr_hospital_4.desease.summary.report'
    _description = 'Summary diseases by month'

    date_from = fields.Date(string='From')
    doctor_ids = fields.Many2many('hr_hospital_4.doctor', string='Doctor(s)')
    year = fields.Integer(
        string='',
        required=False,
        default=2022)
    ...
    def action_open_wizard(self):
        return {
            ...
            'view_mode': 'form',
            'res_model': 'hr_hospital_4.desease.summary.report',
            'target': 'new',
    ...
    def print_report(self):
        ...
        doctors = self.env['hr_hospital_4.doctor'].browse(data['doctor_ids'])
        datas = {
            'ids': [],
            'model': 'hr_hospital_4.doctor',
            'form': data
        }
