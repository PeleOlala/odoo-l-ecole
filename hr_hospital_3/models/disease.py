...
5
    ...
    complete_name = fields.Char(
        ...
        store=True)
    parent_id = fields.Many2one('hr_hospital_3.disease_catalog'
                                , 'Parent catalog'
                                , index=True
                                , ondelete='cascade')
    parent_path = fields.Char(index=True)
    child_id = fields.One2many('hr_hospital_3.disease_catalog'
                               , 'parent_id'
                               , 'Child Class')
    ...
    def _compute_disease_count(self):
        ...
        group_data = dict((data['disease_catalog_id'][0]
                           , data['disease_catalog_id_count']) for data in read_group_res)
        for categ in self:
            disease_count = 0
